...
const InputTextArea = ({ title, placeholder, name, value, onChange, save }) => {
    return <div className="flex flex-col gap-2 p-2 bg-secondary rounded-2xl">
        ...
        <textarea onChange={onChange} name={name} value={value} rows="3" placeholder={placeholder} className='bg-white rounded-lg resize-none border-none outline-none p-2' />
        <button onClick={() => {
            save(name, value)
        }} className='rounded-lg text-green-500 bg-slate-100 min-w-6 h-6 gap-2 px-2 flex items-center justify-center'> Save <IconCheck /> </button>
    </div>
}

const InputLink = ({ saved = false, link, updateLink, removeLink, index
}) => {
    return <div className="flex gap-2 lg:flex-row flex-col">
        <input className='rounded-xl px-2' placeholder='Link Title'
            onChange={(e) => {
                updateLink(index, e.target.value, link.url)
            }}
            value={link.title} />
        <input className='rounded-xl px-2' placeholder='Link Url' value={link.url}
            onChange={(e) => {
        ...
        <button
            ...
            className='rounded-md text-white mt-3 lg:mt-0 px-4 py-1 bg-red-500 font-bold flex items-center justify-center'> <IconTrash /> Delete </button>
    </div>
}

const InputLinkGroup = ({ linkGroup, updateLinkGroup, removeLinkGroup, index
}) => {
    ...
    return <div className="max-w-full w-full min-h-[40px] gap-6 rounded-lg px-2 py-2 bg-blue-400  flex flex-col">
        <div className="flex justify-between lg:flex-row flex-col gap-2">
            <input className='text-[16px] font-bowlby text-black bg-opacity-25 bg-white placeholder:text-white px-1'
                onChange={(e) => { setText(e.target.value) }}
                placeholder='Link Group Title'
                value={text} />
            <div className="flex w-fit gap-2">
                <button
                    onClick={() => { removeLinkGroup(index) }}
                    className='rounded-lg text-red-500 bg-slate-100 min-w-6 h-6 gap-2 px-2 flex items-center justify-center'> Remove Group <IconTrash /> </button>
                <button
                    onClick={() => { updateLinkGroup(index, text, links) }}
                    className='rounded-lg text-green-500 bg-slate-100 min-w-6 h-6 gap-2 px-2 flex items-center justify-center'> Save Group <IconCheck /> </button>
            </div>
        </div>
        {
        ...
        <button
            onClick={() => {
                addLink({
                    title: '', url: '', _id: Math.random() * 1000 + ""
                })
            }}
            title="New Link" className='mx-auto bg-yellow-400 font-bowlby w-8 h-8 flex justify-center items-center rounded-3xl text-[24px]'> + </button>
    </div>
}

const LinkGroups = ({
    linkGroups = [],
    addLinkGroup, updateLinkGroup, removeLinkGroup, addLink, updateLink, removeLink
}) => {
    return <div className="w-full min-h-[40px] gap-4 rounded-lg px-2 py-2 bg-secondary flex flex-col">
        {
            linkGroups.map((linkGroup, index) => {
                return <InputLinkGroup
                    addLink={addLink}
                    updateLink={updateLink}
                    removeLink={removeLink}
                    index={index}
        ...
        <button onClick={() => { addLinkGroup() }} title="New Link group" className=' mx-auto bg-blue-400 font-bowlby w-8 h-8 flex justify-center items-center rounded-3xl text-[24px]'> + </button>
    </div>
}
            ...
            <div className="bg-quaternary gap-4 flex flex-col rounded-2xl mt-14 p-4 min-w-[60%] w-full lg:w-auto min-h-[300px] h-full">
                <div className="flex self-end gap-3">
                    <button
                        ...
                        className='rounded-lg text-primary w-fit self-end bg-slate-100 min-w-6 h-6 gap-2 px-4 py-2 flex items-center justify-center'> Clear <IconClear /> </button>
                    <button
                        onClick={() => {
